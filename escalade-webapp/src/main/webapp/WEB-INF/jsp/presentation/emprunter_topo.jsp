        ...
        <c:forEach items="${topo.listTopoAPreter}" var="topoAPreter">
            <c:if test="${utilisateurConnecte.equals(topoAPreter.utilisateurProprietaire)}">
                <tr class="row">
                    ...
                    <td class="col-xs-7"><a href="${pageContext.request.contextPath}/mes_topos" class="btn btn-success">Mes
                        topos</a>
                    </td>
                </tr>
            </c:if>
            <c:if test="${!utilisateurConnecte.equals(topoAPreter.utilisateurProprietaire)}">
                <tr class="row">
                    <td class="col-xs-5"><a
                            href="mailto:${topoAPreter.utilisateurProprietaire.mail}">${topoAPreter.utilisateurProprietaire.pseudo}</a>
                    </td>
                    <td class="col-xs-7">
                        <c:if test="${topoAPreter.disponible}">
                            <form:form action="addToMy_toposEmpruntes" method="post" class="form-inline">
                                <input type="hidden" id="topoAPreter_id" name="topoAPreter_id"
                                       value="${topoAPreter.utilisateur_topos_id}">
